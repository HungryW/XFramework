...
namespace XFrameworkBase
{
    public sealed partial class CObjectPoolManager : CGameframeworkMoudle, IObjectPoolManager
    {
        ...
        {
            private readonly Dictionary<string, LinkedList<CObject<T>>> m_mapFreeObj;
            private readonly Dictionary<object, CObject<T>> m_ObjectMap;
            ...
            public CObjectPool(string name, bool allowMultiSpawn, float autoReleaseInterval, int capacity, float expireTime, int priority)
                : base(name)
            {
                m_mapFreeObj = new Dictionary<string, LinkedList<CObject<T>>>();
                m_ObjectMap = new Dictionary<object, CObject<T>>();
            ...
            public void Register(T obj, bool spawned)
            {
                ...
                CObject<T> internalObject = CObject<T>.Create(obj, spawned);
                m_ObjectMap.Add(obj.Target, internalObject);
                if (!spawned || AllowMultiSpawn)
                {
                    var stack = _GetOrAddFreeStack(obj.Name);
                    stack.AddLast(internalObject);
                }
            ...
            private LinkedList<CObject<T>> _GetOrAddFreeStack(string a_szName)
            {
                LinkedList<CObject<T>> stack = null;
                m_mapFreeObj.TryGetValue(a_szName, out stack);
                if (stack == null)
                {
                    stack = new LinkedList<CObject<T>>();
                    m_mapFreeObj.Add(a_szName, stack);
                }
                return stack;
            }
            ...
            {
                var stack = _GetOrAddFreeStack(name);
            ...
            public void Unspawn(object target)
            {
                ...
                CObject<T> internalObject = GetObject(target);
                Debug.Assert(internalObject != null);
                internalObject.Unspawn();

                if (!m_AllowMultiSpawn)
                {
                    var stack = _GetOrAddFreeStack(internalObject.Name);
                    stack.AddLast(internalObject);
                }
            ...
            public bool ReleaseObject(object target)
            {
                ...
                var stack = _GetOrAddFreeStack(internalObject.Name);
                stack.Remove(internalObject);
                internalObject.Release(false);
                CReferencePoolMgr.Release(internalObject);
                return true;
            }
            ...
            {
                foreach (KeyValuePair<object, CObject<T>> objectInMap in m_ObjectMap)
                {
                    objectInMap.Value.Release(true);
                    CReferencePoolMgr.Release(objectInMap.Value);
                }
                m_mapFreeObj.Clear();
                m_ObjectMap.Clear();
            ...
            private CObject<T> GetObject(object target)
            {

                CObject<T> internalObject = null;
            ...
            private void GetCanReleaseObjects(List<T> results)
            {
                ...
                foreach (KeyValuePair<object, CObject<T>> objectInMap in m_ObjectMap)
                {
                    CObject<T> internalObject = objectInMap.Value;
                    if (internalObject.IsInUse || internalObject.Locked || !internalObject.CustomCanReleaseFlag)
                    {
                        continue;
                    }
